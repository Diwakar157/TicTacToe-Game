...
🧱 OOP Concepts Used
Concept	Where Used
Classes & Objects	`TicTacToe`, `HumanPlayer`, `AIPlayer`
Abstract Class	`Player` — defines `makeMove()` contract
Inheritance	`HumanPlayer` and `AIPlayer` extend `Player`
Encapsulation	Board state managed inside `TicTacToe`
Polymorphism	`Player cp` holds either `HumanPlayer` or `AIPlayer`
---
🚀 How to Run
Prerequisites
